-- SolarFlow: schema inicial con RLS
...
-- Parámetros tarifa anual (nacional)
CREATE TABLE parametros_tarifa_anual (
...
CREATE INDEX idx_localidades_ccaa ON localidades(ccaa);
...
-- Localidades: lectura pública
CREATE POLICY "localidades_select_public" ON localidades
  FOR SELECT USING (activo = true);

-- Parametros tarifa: lectura pública
...
-- Widget público: leer empresa por slug (solo campos branding + estado activo)
...
-- Trigger: incrementar leads_usados_mes
