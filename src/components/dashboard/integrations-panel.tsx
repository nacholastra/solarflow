...
import type { IntegrationsData } from "@/lib/dashboard/integrations-data";
...
export function IntegrationsPanel({ data }: { data: IntegrationsData }) {
  ...
  async function handleTest() {
    ...
    if (!res.ok) {
      toast({ variant: "destructive", title: "Error en la prueba", description: json.error });
      return;
    }
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <Card className={!webhooksEnabled ? "opacity-60" : undefined}>
        <CardHeader>
          ...
          <CardDescription>
            Cada vez que un cliente complete el simulador y envíe sus datos, recibirás un POST JSON con toda la
            información del lead en la URL que configures aquí.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {webhooksEnabled ? (
