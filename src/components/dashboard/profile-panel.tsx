...
import type { ProfileData } from "@/lib/dashboard/profile-data";
...
export function ProfilePanel({ profile, appUrl }: { profile: ProfileData; appUrl: string }) {
  const router = useRouter();
  const [nombreEmpresa, setNombreEmpresa] = useState(profile.empresa.nombre_empresa);
  ...
  function openDeleteDialog() {
    if (confirmDelete.trim() !== empresa.nombre_empresa.trim()) {
      toast({
        variant: "destructive",
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-sm capitalize text-muted-foreground">{empresa.estado_suscripcion}</span>
          </div>
