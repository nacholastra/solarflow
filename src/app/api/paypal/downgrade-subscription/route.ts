...
export async function POST(request: Request) {
  try {
    ...
    if (empresa.plan !== "pro") {
      return NextResponse.json({ error: "No tienes el plan Pro" }, { status: 400 });
    }

    if (empresa.estado_suscripcion !== "active") {
      return NextResponse.json({ error: "Activa un plan antes de cambiar" }, { status: 400 });
    }

    const currency = empresa.moneda_facturacion ?? "EUR";
    const basicPlanId = getPayPalPlanId("basic", currency);

    if (!basicPlanId) {
      return NextResponse.json({ error: "Plan Basic no configurado en PayPal" }, { status: 500 });
    }
    ...
    if (!body.subscriptionId && subscriptionId) {
      ...
      const reviseRes = await fetch(
        `${getPayPalApiBase()}/v1/billing/subscriptions/${subscriptionId}/revise`,
        {
          ...
          body: JSON.stringify({ plan_id: basicPlanId }),
        },
      );
    ...
    if (!subscriptionMatchesPlan(subscription, "basic", currency)) {
      return NextResponse.json({ error: "El plan de PayPal no es Basic" }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: true, plan: "basic" });
  } catch (e) {
